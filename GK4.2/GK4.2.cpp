// GK4.2.cpp : Defines the entry point for the console application.
//
...
GLuint GRASS, OAKTREE;

// identyfikatory list wyświetlania

GLint GRASS_LIST, OAKTREE_LIST;
...
void DisplayScene()
{
	...
	glBindTexture(GL_TEXTURE_2D, OAKTREE);
	glPushMatrix();
	glTranslatef(-0.5, 0.0, 0.0);
	glScalef(0.5, 0.5, 0.5);
	glCallList(OAKTREE_LIST);
...
void GenerateTextures()
{
	...
	// wczytanie tekstury oak_tree.tga
	error = load_targa("maple_tree_small.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku maple_tree_small.tga");
		exit(0);
	}

	// utworzenie identyfikatora tekstury
	glGenTextures(1, &OAKTREE);
...
void GenerateDisplayLists()
{
	...
	OAKTREE_LIST = glGenLists(1);

	// druga lista wyświetlania - drzewo
	glNewList(OAKTREE_LIST, GL_COMPILE);
